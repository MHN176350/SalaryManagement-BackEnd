...
namespace Project.Repositories.Impl
{
    public class PayrollRepository : IPayrollRepository
    {
        ...
        public bool CheckEmployeeAlreadyHasPayroll(DateTime date, int empId)
        {
            ...
            {
                if ((payroll.StartDate.Date.Month == date.Date.Month && payroll.StartDate.Date.Year == date.Date.Year) && (payroll.Status == Enum.EnumList.PayrollStatus.Approved || payroll.Status == Enum.EnumList.PayrollStatus.Waiting))
        ...
        public List<int> CreatePayroll(PayrollReq req)
        {
            ...
            int days = AutoEmployee.GetTotalDayInMonth(req.dateTime.Date.Year, req.dateTime.Date.Month);
            ...
            if (contractInMonth.Count > 0)
            {
                foreach (var contract in contractInMonth)
                {
                    ...
                    if (contract.EmployeeType == Enum.EnumList.EmployeeType.FullTime)
                    {
                        ...
                        payroll.Total = AutoEmployee.TotalPayroll(payroll.BaseSalaryPerHours, (decimal)payroll.RealWorkingHours, payroll.OTSalaryPerHours, (decimal)payroll.OTWorkingHours, dayTakeLeave, payroll.Bonus);
                    }
                    else
                    {
                        ...
                        payroll.Total = AutoEmployee.TotalPayroll(payroll.BaseSalaryPerHours, (decimal)payroll.RealWorkingHours, payroll.OTSalaryPerHours, (decimal)payroll.OTWorkingHours, 0, payroll.Bonus);
                    }
                    if (contract.SalaryType == Enum.EnumList.SalaryType.Gross)
                    {
