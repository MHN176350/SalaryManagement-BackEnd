...
namespace Project.Repositories.Impl
{
    public class TakeLeaveRepository : EfEntityRepositoryBase<TakeLeave, MyDbContext>, ITakeLeaveRepository
    {
        ...
        public bool existApprovedAttendanceByDateEqualAndEmployeeEqual(int employeeId, DateTime startDate, DateTime endDate) => _attendenceRepository.GetFirstOrDefault(a => a.Date.Date >= (startDate.Date) && a.Date.Date <= endDate.Date && a.EmployeeId == employeeId && a.Status.Equals(EnumList.AttendanceStatus.Approved)) != null;
    }
}
